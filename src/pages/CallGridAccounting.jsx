...
/** Normalize CallGrid buyers API payload to display strings (buyerName). */
function parseCallgridBuyersListResponse(json) {
  if (json == null) return [];
  const nested = json.buyers ?? json.data ?? json.items;
  if (!Array.isArray(nested)) return [];
  return nested
    .map((item) => {
      if (typeof item === "string") return item.trim();
      if (item && typeof item === "object") {
        const n = item.buyerName ?? item.name ?? item.buyer ?? item.label;
...
function parseInputDateStr(dateStr) {
...
/** Filter cached revenue rows to the selected date range (inclusive). */
function filterRevenueByDateRange(revenue, startStr, endStr) {
  if (!Array.isArray(revenue)) return [];
  const start = parseInputDateStr(startStr);
  const end = parseInputDateStr(endStr);
  if (!start || !end) return revenue;
  return revenue.filter((dayEntry) => {
    const d = parseDayStrToDate(dayEntry.day);
...
function buildBuyerTable(revenue, allCompanies = []) {
  ...
  const buyerHasParagonData = new Set();
  ...
  const buyers = Object.keys(buyerMap)
    .filter((b) => buyerHasComparison.has(b) || buyerHasParagonData.has(b))
    .sort();
...
function CallGridAccounting() {
  ...
  const fetchRevenue = useCallback(async () => {
    ...
    setCacheEmpty(false);
    try {
      const res = await fetch(API_ENDPOINTS.CALLGRID_ACCOUNTING.REVENUE_CACHED, {
        method: "GET",
      ...
      const payload = Array.isArray(json) && json[0]?.companies ? json[0] : json;
      if (payload?.companies?.[0]) {
        const company = payload.companies[0];
        const filteredRevenue = filterRevenueByDateRange(
          company.revenue ?? [],
          start,
          end,
        );
        setData({
          ...payload,
          companies: [{ ...company, revenue: filteredRevenue }],
        });
      } else {
        setData(payload);
      }
      ...
      try {
        ...
        if (!cancelled && Array.isArray(list)) {
          setCompaniesDirectory(
            list.filter((c) => companyPlatformFromDoc(c) === "callgrid"),
          );
        }
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const buyersUrl = `${API_ENDPOINTS.CALLGRID_ACCOUNTING.BUYERS}?days=30`;
        const res = await fetch(buyersUrl, {
          method: "GET",
          headers: getAuthHeaders(),
        });
        if (cancelled) return;
        ...
        const json = await res.json().catch(() => null);
        if (cancelled) return;
        const raw = parseCallgridBuyersListResponse(json);
        const seen = new Set();
        const deduped = [];
  ...
  const publisherCompany = companies[0] ?? null;
  const tables = publisherCompany
    ? [
        {
          companyName: publisherCompany.companyName ?? "Paragon Media",
          rows: buildBuyerTable(
            publisherCompany.revenue ?? [],
            companiesForLookup,
          ),
        },
      ]
    ...
    refreshPollRef.current = setInterval(async () => {
      try {
        const res = await fetch(
          API_ENDPOINTS.CALLGRID_ACCOUNTING.REVENUE_REFRESH_STATUS,
          { headers: getAuthHeaders() },
        );
        if (!res.ok) return;
        ...
        stopRefreshPoll();
        setRefreshing(false);
        setRefreshError(err?.message || "Failed to check refresh status.");
      }
    }, REFRESH_POLL_MS);
  }, [fetchRevenue, stopRefreshPoll]);
  ...
  const handleRefreshData = async () => {
    ...
    setCacheEmpty(false);
    try {
      const res = await fetch(API_ENDPOINTS.CALLGRID_ACCOUNTING.REVENUE_REFRESH, {
        method: "POST",
        headers: getAuthHeaders(),
      });
      ...
      setRefreshing(false);
      setRefreshError(err?.message || "Failed to start refresh.");
    }
  };

  const formatCacheTimestamp = (iso) => {
    if (!iso) return "";
    try {
  ...
  const isCallgridAddBuyerBlocked =
    callgridResolveStatus !== "success" || !addBuyerForm.accountID?.trim();
  ...
  const handleAddBuyerSubmit = async (e) => {
    ...
    if (callgridResolveStatus !== "success" || !accountIDForSubmit) {
      const result = await resolveCallgridFromApiToken(addBuyerForm.apiToken);
    ...
    try {
      const body = {
        ...
        platform: "callgrid",
        apiToken: addBuyerForm.apiToken.trim(),
      };
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            ...
            <div>
              ...
              {cacheMeta?.refreshedAt && (
                <p className="text-gray-500 text-sm mt-1">
                  Last refreshed: {formatCacheTimestamp(cacheMeta.refreshedAt)}
                  {cacheMeta.windowStart && cacheMeta.windowEnd && (
                    <>
                      {" "}
                      · Cache window:{" "}
                      {formatDateRange(
          ...
          {refreshError && (
            <p className="text-red-600 mb-4 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-sm">
              {refreshError}
            </p>
          )}
          ...
          {!loading && cacheEmpty && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-6 text-center mb-4">
              <p className="text-amber-900 font-medium mb-2">
                No cached CallGrid revenue yet
              </p>
              <p className="text-amber-800 text-sm mb-4">
                Run a refresh to pull Paragon Media CallGrid stats and buyer
                cross-checks. This may take a few minutes.
              </p>
              <button
                type="button"
                onClick={handleRefreshData}
          ...
          {!loading && !error && !cacheEmpty && tables.length === 0 && (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label
                      ...
                      className="block text-sm font-medium text-gray-700 mb-1"
                    >
                      CallGrid API Token <span className="text-red-500">*</span>
                    </label>
                    <input
                      ...
                      placeholder="Paste buyer CallGrid API key"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Resolved after valid API key"
                      disabled={callgridResolveStatus === "loading"}
                    />
              ...
              <form onSubmit={handleEditBuyerSubmit} className="p-6">
                ...
                {!editBuyerOriginalAccountId.trim() && (
                  <p className="text-amber-700 text-sm mb-4 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                    No CallGrid Organization ID was found for this row. Saving is
...
export default CallGridAccounting;
